...
import com.xX_deadbush_Xx.hocus.api.ritual.RitualManager;
...
public class KillMobTask extends RitualTask<LivingDeathEvent> {
	
	public KillMobTask(int ticksuntilexpire, boolean necessary) {
	...
	public static void onKill(LivingDeathEvent event) {
		if(event.getSource().getTrueSource() instanceof PlayerEntity) {
			if(event.getEntityLiving() instanceof AnimalEntity) {
				RitualManager.getInstance().handleTaskEvent(event, KillMobTask.class);
			}
		}
	}
